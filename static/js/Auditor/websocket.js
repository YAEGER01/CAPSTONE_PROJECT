(function () {
  ...
  // Robust refresh helper: prefer global window.refreshAll, then local refreshAll,
  // otherwise emit an event so the dashboard can respond when loaded.
  function doRefresh() {
    try {
      ...
      // swallow errors - we'll fallback to event dispatch below
    }
    try {
      document.dispatchEvent(new Event('auditor:refresh'));
    } catch (e) {}
  }
